        ...
        <table <?php echo  @$this->uri->segment(1)=='efile' && $this->uri->segment(2)!='' ? '' : "id='view_table'" ?> class="table table-bordered table-striped display">
            ...
            <?php $i=1; foreach ($get_files as $key => $files) {
                ...
                ?>>
                    ...
                    <td width="150px">
						<?php echo $this->uri->segment(1) =='e-files' && $files->file_hardcopy_status == 'received' && emp_session_id() == $files->file_received_emp_id   ? '<a href="'.base_url().'efile/'.$files->file_id.'" class="btn btn-sm btn-block btn-warning"> ई- फाइल देखें</a>'  : false; ?>
                        ...
                        if($senderemp[0]['role_id'] == '9' || $senderemp[0]['role_id'] == '8' || $this->uri->segment(3) == ''){
                            if($files->file_hardcopy_status == 'not'){
								...
								if((isset($is_draft_detail['draft_status']) && ($is_draft_detail['draft_status']==3 || $is_draft_detail['draft_status']==2)) || ($this->uri->segment(1)=='view_file' && strtolower($files->file_status)=='p')){
									/*Fore E-File*/
									echo  '<button onclick="open_model2('.$files->file_id.',&#39;'.$files->file_status.'&#39;)" value="'.$files->file_id.'"  class="btn btn-sm btn-block btn-twitter upperuser" data-toggle="tooltip" data-original-title="Send to officer"><i class="fa fa-send"></i> अधिकारी को भेजें</button>' ;
								}  
								...
								if($files->file_return == '2' && $files->file_hardcopy_status != 'not'){
										...
										echo   $files->file_hardcopy_status == 'received' && emp_session_id() == $files->file_received_emp_id && $files->file_level_id != '31' ? '<button onclick="open_model2('.$files->file_id.',&#39;'.$files->file_status.'&#39;)" value="'.$files->file_id.'"  class="btn btn-sm btn-block btn-twitter upperuser" data-toggle="tooltip" data-original-title="Send to officer"><i class="fa fa-send"></i> अधिकारी को भेजें</button> ' : false;
                                ...
                                <div class="col-xs-12">
                                    ...
                                    <div class="form-group checkbox">
                                         ...
                                         <input type="checkbox" id="physical_file_1" name="file_status[]" class="form-group physical_file" value="p"  >Physical File
											...
											<input type="hidden"  name="file_status[]" class="form-group" value="p"  >
											<?php } ?>
									   </label>
                                        <label>
                                           <input type="checkbox" id="electronic_file_1" name="file_status[]" value="e" class="form-group electronic_file" >E-File
											<?php if(($this->uri->segment(1)== 'e-files') || ($this->uri->segment(1)== 'efile')){ ?>
											<input type="hidden"  name="file_status[]" class="form-group" value="e"  >
											<?php } ?>
                                        </label>
											<div id="efile_div"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
