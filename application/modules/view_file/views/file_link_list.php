...
<?php if($file_details[0]['file_received_emp_id'] == emp_session_id() && $file_details[0]['file_hardcopy_status']!='not') {?>
    ...
    <form action="<?php echo base_url(); ?>view_file/file_search/manage_link_files" method="post">
                    ...
                    $i=1; foreach ($get_files_related as $key => $files) {
                        ...
                        if($files != null && $files->file_id != $file_details[0]['file_id'] && $files->file_hardcopy_status != 'close') {?>
                            <tr class="text-bold" data-toggle='tooltip' data-original-title="Related files">
                                ...
                                <td><?php  if((emp_session_id() == $files->file_received_emp_id && $files->file_hardcopy_status == 'received') || $files->file_hardcopy_status == 'close') {?>
                                ...
                                <td align=""><?php
                                    ...
                                    if ($filereceiver)
                                    {
                                        if($files->file_hardcopy_status == 'not') {
                                            ...
                                            echo "<span style='color:#31708f' >Closed by <b>".ucfirst($filereceiver[0]->emp_full_name)."</b><br/>(".$filereceiver[0]->emprole_name_hi.")</span>";
                                        }else{
        ...
        <div class="col-md-12"><button type="submit" onclick="return confirm('क्या आप नस्ती को एकीकृत करना चाहते है |')" class="btn btn-success" <?php echo $disable_btn ?> id="merge_id">MERGE FILES</button>
            <span class="text-red"> * एकीकृत होने  के बाद फाइल अलग नहीं होगा |</span></div>
