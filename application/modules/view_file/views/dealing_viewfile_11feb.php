										...
										<?php
                                            ...
                                            <?php if (($files->file_hardcopy_status == 'received' || $files->file_hardcopy_status == 'working') && $files->file_return != '1') {
												...
												?>
													...
													<button type="button" onclick="open_model_csu(<?php echo $files->file_id; ?>);" class="btn btn-block btn-sm btn-instagram" data-toggle="tooltip" data-original-title="Mark to scan unit for add PDF">Mark to CSU</button>
                                                <?php }else{ ?>
...
function receive_DA(file,file_status){
	...
	if(file_status == 'p,e'|| file_status == 'e,p'){
        ...
        $('#form_submit_link').attr('action','<?php echo base_url() ;?>view_file/dealing_file/receive_file_da/'+file2);
    }
