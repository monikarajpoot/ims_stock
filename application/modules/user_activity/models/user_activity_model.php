...
class User_activity_model extends CI_Model {
    ...
    function get_activity_list($id = '') {
        ...
        $this->db->select($count . "$login_log_table.emp_id, emp_unique_id, emp_full_name,emp_full_name_hi, role_id, log_browser,log_ip_address, log_create_date");
    ...
    function get_activity_list_not_login(){
        $user_login_log = EMPLOYEE_LOGIN_LOG;
        $employee = EMPLOYEES;
        $this->db->select("distinct(emp_unique_id), emp_full_name,emp_full_name_hi, role_id");
        $this->db->from($employee);
    ...
    function get_activity_list_today() {
        ...
        $user_login_log = EMPLOYEE_LOGIN_LOG;
        $employee = EMPLOYEES;
        $this->db->select("DISTINCT($login_log_table.emp_id), emp_unique_id, emp_full_name,emp_full_name_hi, role_id, log_browser,log_ip_address, log_create_date");
        $this->db->from($employee);
        $this->db->join($user_login_log, $user_login_log . '.emp_id =' . $employee . '.emp_id');
        $this->db->where($where);
        $this->db->where("role_id !=", 1);
