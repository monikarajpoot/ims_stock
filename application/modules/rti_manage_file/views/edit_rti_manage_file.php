    ...
    <div class="box box-primary">
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="file_type" id="file_type">
                    <?php foreach(file_types() as $key => $value){
                        if($file_detail[0]['file_type']==$key){
							$auto_select='selected';
						}else if($file_type==$key){
                            $auto_select='selected';
                        }
                        if($key=='f' || $key=='l' || $key=='n' ){
			...
			<div class="form-group">
                ...
                <input type="text" name="rti_file_applicant_firm_contact_no" required value="<?php echo isset($firm_contact_no) ? $firm_contact_no : ''; ?>" placeholder="<?php echo $this->lang->line('rti_file_applicant_firm_contact_no'); ?>" class="form-control">
                <?php echo form_error('rti_file_applicant_firm_name');?>
            </div>
            <div class="form-group">
            ...
            <?php if($file_type!='n'){ ?>
                <div class="form-group hide_for_notice" id="" >
                    <label for="RTI_fees" id="RTI_fees"><?php echo $this->lang->line('rti_file_fee')?></label> <span class="text-danger">*</span></label>
			...
			<div class="form-group" id="">
                <label for="RTI_fees" id="RTI_tip"><?php echo $this->lang->line('RTI_tip')?></label></label>
...
<div class="col-md-6">
    <div class="box box-primary">
        <div class="box-body">
            ...
            <div class="form-group hide_for_notice">
                ...
                <select class="form-control" name="file_offer_by" id="file_offer_by">
                    ...
                    <?php foreach(file_from_type() as $key => $value){ ?>
                        <option value="<?php echo $key ?>" <?php if ($this->input->post('file_offer_by') == $key || $file_detail[0]['file_offer_by'] == $key ) { echo "selected";} ?>><?php echo $value ?></option>
                    <?php   } ?>
                </select>
            ...
            <?php }?>
            <div class="form-group"  id="High_court_show" <?php if($this->input->post('file_offer_by') == 'm' || $this->input->post('file_offer_by') == 'u' || $file_detail[0]['file_Offer_by']=='u' || $file_detail[0]['file_Offer_by']=='u') { echo "style='display: block'"; } else { echo "style='display: none'";} ?>>
                ...
                <select class="form-control" name="court_bench" id="court_bench">
                    ...
                    <?php foreach(highcourt_bench() as $key => $value){ ?>
                        <option value="<?php echo $key ?>" <?php if ($this->input->post('court_bench') == $key || $file_detail[0]['court_bench_id']==$key) { echo "selected";} ?>><?php echo $value ?></option>
                    <?php  } ?>
                </select>
                <?php echo form_error('court_bench');?>
            </div>
            <div class="form-group" id="dept_id_show" <?php if($this->input->post('file_offer_by') == 'v' || $this->input->post('file_offer_by') == 'v' || $file_detail[0]['file_Offer_by']=='v') { echo "style='display: block'"; } else { echo "style='display: none'";} ?>>
                ...
                <select class="form-control" name="file_department_id" id="file_department_id">
                    ...
                    <?php foreach($departments_list as $row){ ?>
                        <option value="<?php echo $row['dept_id']; ?>" <?php  if($this->input->post('file_department_id')==$row['dept_id'] || $file_detail[0]['file_department_id']==$row['dept_id']){ echo 'selected';} ?>><?php echo $row['dept_name_en']." ".$row['dept_name_hi']; ?></option>
                    <?php } ?>
                    <option value="400"><?php echo $this->lang->line('option_other'); ?></option>
                </select>
                <?php echo form_error('file_department_id');?>
            </div>
            <div class="form-group" id="dist_id_show" <?php if($this->input->post('file_offer_by') == 'c' || $this->input->post('file_offer_by') == 'jvn' || $file_detail[0]['file_Offer_by']=='jvn' || $file_detail[0]['file_Offer_by']=='c') { echo "style='display: block'"; } else { echo "style='display: none'";} ?>>
                ...
                <select class="form-control" name="district_id" id="district_id">
                    ...
                    <?php foreach($district_list as $row){ ?>
                        <option value="<?php echo $row['district_id']; ?>" <?php  if($this->input->post('district_id')==$row['district_id'] || $file_detail[0]['district_id'] == $row['district_id']){ echo 'selected';} ?>><?php echo $row['district_name_en'].", ".$row['district_name_hi']; ?></option>
                    <?php } ?>
                    <option value="400"><?php echo $this->lang->line('option_other'); ?></option>
                </select>
                <?php echo form_error('district_id');?>
            </div>
            <div class="form-group" id="state_id_show" <?php if($this->input->post('file_offer_by') == 'au' || $file_detail[0]['file_Offer_by']=='au') { echo "style='display: block'"; } else { echo "style='display: none'";} ?>>
                <label for="district_id"><?php echo $this->lang->line('label_state_name'); ?> <span class="text-danger">*</span></label>
                <select class="form-control" name="state_id" id="state_id">
                    <option value="">Select</option>
                    <?php foreach($state_list as $row){ ?>
                        <option value="<?php echo $row['state_id']; ?>" <?php  if($this->input->post('state_id')==$row['state_id'] || $file_detail[0]['state_id'] == $row['state_id']){ echo 'selected';} ?>><?php echo $row['state_name_en']." ".$row['state_name_hi']; ?></option>
                    <?php } ?>
                </select>
			...
			<div class="form-group">
                ...
                <select class="form-control marktosc" required name="mark_file_section_id" id="mark_file_section_id" style="display:<?php echo $fdis;?>">
                        <option value="" id="select_t_s"><?php echo $this->lang->line('option_select_section'); ?></option>
                        <?php foreach($section_list as $row){ 
							if($row['section_id']!=26 && $row['section_id']!=21 && $row['section_id']!=1){
						?>  <option value="<?php echo $row['section_id']; ?>" class="sections" <?php  if($this->input->post('file_mark_section_id')==$row['section_id'] || $file_detail[0]['file_mark_section_id']== $row['section_id']){ echo 'selected';} ?>><?php echo $row['section_name_en'].", ".$row['section_name_hi']; ?></option>
						<?php } } ?>
                </select>
