...
class Draft extends MX_Controller {
	...
	function search_file($location = null, $draft_id = null){
        ...
        if ($this->form_validation->run($this) === TRUE) {
			...
			$data['file_data'] = getFiledata($data_file['file_id']);
	...
	function add_fileto_draft($file_id, $draft_id = null){
		...
		$data['add_draft']    = true;
		$data['search_file']    = true;
		$data['file_data'] =  getFiledata($file_id);
		$data['module_name']    = "draft";
		if($draft_id != null){
			$final_data = array(
	...
	function complete_draft($id = ''){
		...
		$file_data = getFiledata($draft_data['draft_file_id']);
	...
	function delete_draft($id, $file_id){
		//pr($id);
		$res = delete_data(DRAFT, array('draft_id' => $id));
		$res = delete_data(DRAFT_LOG, array('draft_log_draft_id' => $id));
		if($res){
			$file_data =  getFiledata($file_id);
	...
	function delete_draft_log($id, $file_id){
		...
		$res = delete_data(DRAFT_LOG, array('draft_log_id' => $id));
		$res = delete_data(DRAFT, array('draft_id' => $d_id));
		if($res){
			$file_data =  getFiledata($file_id);
			$filesin_draft = explode(',',$file_data[0]['final_draft_id']);
