...
<div class="col-md-12">
    <form method="post" action="">
        <button type="submit" value="related_view" name="show_related_view" class="btn btn-block btn-vk">Click here for related view</button>
    </form>
    ...
    <input type="hidden" name="file3" value="<?php echo $file_details[0]['file_id'] ; ?>">
               <div class="col-md-12" style="overflow: auto">
                <table class="table table-bordered table-hover">
                    ...
                    $i=1; foreach ($get_files_related as $key => $files) {
                        ...
                        if($files != null && $files->file_id != $file_details[0]['file_id']) {?>
            ...
            if(isset($get_files) && !empty($get_files)){
                    $i=1; foreach ($get_files as $key => $files) {
                        ...
                        if($files != null && $files->file_id != $file_details[0]['file_id']) {?>
                            <tr <?php if(emp_session_id() != $files->file_received_emp_id && $files->file_hardcopy_status != 'close') {?> data-toggle='tooltip' data-original-title="कृप्या इस फाइल को आहुत करें |" <?php } ?>>
                                <td>
                                 <?php  if(emp_session_id() == $files->file_received_emp_id || $files->file_hardcopy_status == 'close') {?>
                                    <input type="checkbox" name="linkcheck[]" value="<?php echo $files->file_id;?>"/>
            ...
            <div class="col-md-3"><button type="submit" class="btn btn-success" <?php echo $disable_btn ?>>MERGE FILES</button></div>
</form>
</div><!-- /.box-body -->
</div><!-- /.box -->
</div>
