                    ...
                    <?php if(isset($get_files)) {
                        $i=1; foreach ($get_files as $key => $files) { ?>
                            <tr>
                                ...
                                <td> <?php $section_number_array= get_cr_section_nuber($files->file_id); ?>  <?php echo $i;?> <span style="display:none">(<?php echo $this->lang->line('file_no'); ?> : <?php echo $files->file_id;?>)</span> <?php if(isset($section_number_array['section_number']) && $section_number_array['section_number']!='') {?><br/> CR-<?php echo $section_number_array['section_number'];?><?php } ?></td>
                                <td><?php echo $files->file_subject;?><br><?php show_scan_file($files->scan_id)?></td>
                                ...
                                <td align="center" class="no-print">
                                    ...
                                    echo   $files->file_hardcopy_status == 'received' && emp_session_id() == $files->file_received_emp_id && emp_session_id() != '2' ? '<button onclick="open_model2('.$files->file_id.')" value="'.$files->file_id.'" class="btn btn-sm btn-block btn-info upperuser">'.$this->lang->line('send_file_officer').'</button>' : false;
                                    ...
                                    echo   $files->file_hardcopy_status == 'not' && emp_session_id() == $files->file_received_emp_id ? '<button onclick="open_model3('.$files->file_id.')" class="btn btn-block btn-sm btn-twitter"><span class="blink_fast">'.$this->lang->line('receive_file').'</span></button>' : false;
									echo   $files->file_hardcopy_status == 'not' && emp_session_id() == $files->file_received_emp_id ? '<a data-toggle="tooltip" data-original-title="अस्वीकार करने पे यह फाइल पिछले इस्तर पे आ जाय गई |" onclick="return confirm_reject()" href="'.base_url().'manage_file/send_file_return/'.$files->file_id.'" class="btn btn-block btn-sm btn-instagram" data-toggle="tooltip" data-original-title="Reject">Reject</a>' : false;
        ...
        $(".rty").click(function () {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var otpt = '<select class="form-control" required id="check_sec_count" name="emp_id"><option value="" >चयन करें </option>';
					...
					//otpt += '<option value="133" null="">डॉ. दीपक डेनियल पगारे (प्रभारी)   गोपनीय</option>';
                    otpt += '</select>';
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(r_data, function( index, value ) {
                        ...
                        otpt1 += '<option value="'+value.emp_id+'">'+fword_hi+' '+value.emp_full_name+' ('+value.emprole_name_hi+')</option>';
                    });
                    otpt1 += '</select>';
