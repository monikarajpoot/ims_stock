	...
	var HTTP_PATH='<?php echo base_url(); ?>';
		...
		success: function(data){
			...
			// console.log(data);
			  ...
			  var r_data = JSON.parse(data);
			 // console.log(r_data);
			  // console.log(r_data[0]['once_registration_number_council']);
			   ...
			   $("#chalan_no").val(r_data[0]['adv_invoice_no']);
				// $("#adv_appointment_posting_date").val(r_data[0]['once_registration_number_council']);
				//  $("#post_renew_date").val(r_data[0]['once_registration_number_council']);
		}
		});
...
function advocate_post_data(){
	...
	// if($("#once_registration_number_council").val() == '' ){
		// $("#once_registration_number_council-error").html("कृपया इसे जरुर दर्ज करे!");
		// flag = 0; 
	// }
