												...
												if (isset($notesheets) && !empty($notesheets)) {
													foreach ($notesheets as $nkey => $nvalue) {
														...
														<?php if (isset($file_dat_show)) { 
															...
															$file_id = $file_dat_show[0]['file_id'];
																 ?>
															<a onclick="get_notery_form(<?php echo $file_id ?>,'<?php echo $url; ?><?php echo $nvalue['notesheet_id'] . '/' . $docdata['section_id'] . '/' . $file_id; ?>')"  style="curser:pointer" <?php echo $cls; ?>  >
				...
				?>
					<div class="row">
						...
						<?php  if(empty($draft_data)){ ?>
							<a href="<?php echo base_url().'draft/create_fileto_draft/'.$file_dat_show[0]['file_id'].'/n'; ?>" class="btn btn-primary btn-flat"> नोटशीट </a>
						<?php } else{
							echo '<p class="bg-danger">आप पहले से ही इस फाइल पर नोट शीट जोड़ चुके ह आब नहीं जोड़ सकते क्रपया देखे या एडिट करे| <a href="'.base_url().'efile/'.$file_dat_show[0]['file_id'].'" class="btn btn-primary btn-flat"> नोटशीट </a> </p>';
						} ?>	
						</div>
					</div><hr/>
					<div class="row">
						<div class="col-md-12"><h4>विभाग को भेजने वाला  पत्राचार Correcpondance- C जोड़े</h4> </div>
						<div class="col-md-2 text-center">
							<a href="<?php echo base_url().'draft/create_fileto_draft/'.$file_dat_show[0]['file_id'].'/o'; ?>" class="btn btn-warning btn-flat">आर्डर</a>
						</div>
						<div class="col-md-2 text-center">
							<a href="<?php echo base_url().'draft/create_fileto_draft/'.$file_dat_show[0]['file_id'].'/r'; ?>" class="btn btn-danger btn-flat">स्मरण पत्र</a>
						</div>
						<div class="col-md-2 text-center">
							<a href="<?php echo base_url().'draft/create_fileto_draft/'.$file_dat_show[0]['file_id'].'/ot'; ?>" class="btn btn-success btn-flat">अन्य</a>
						</div>
					</div>
				</div>
			</div>
