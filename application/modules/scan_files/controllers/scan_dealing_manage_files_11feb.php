...
class scan_dealing_manage_files extends MX_Controller {
    ...
    //        $this->form_validation->set_rules('file_department_id', $this->lang->line('file_department'), 'trim|required|xss_clean');
        ...
        {
			$scanfile_IDSval = explode(',',$this->input->post('scan_files_id'));
			foreach($scanfile_IDSval as $scan_ids){
				if(!empty($scan_ids)){
            ...
            $data_dealing = array(
                ...
                'file_department_id'        => $this->input->post('file_department_id')!=''?$this->input->post('file_department_id'):0,
                'file_district_id'          => $this->input->post('district_id')!=''?$this->input->post('district_id'):0,
			...
			if(count($scanfile_ids)>0){
								...
								if(isset($scan_id_exits) && !empty($scan_id_exits) )
								{
									...
									if(is_array($existids)){
									$result = array_merge($existids,$scanfile_ids);
									
								
									$data_dealing['scan_id'] =  serialize($result);
									}
									...
									$scan_ids = $scanfile_ids;
									$data_dealing['scan_id'] = serialize($scan_ids);
									}
							}
			
			//$scan =  unserialize('a:3:{i:0;s:2:"22";i:1;s:2:"23";i:2;s:2:"24";}');
			//print_r($scan);
			//pr( $data_dealing);
            ...
            $data_dealing_log = array(
				...
				'scan_ids'			=> serialize($scanfile_ids)
            );
//pr( $data_dealing_log);
         ...
         //   pr($file_other_feilds);
            if($id){
                ...
                if($res){
                    ...
                    $this->session->set_flashdata('message', 'फाइल/पत्र क्रं:&nbsp;'.$id.'&nbsp; सफलतापूर्वक  प्रगति पर आ चुकी है , अन्य दस्तावेज़ संलग्न करने के लिए  <a href="'.base_url().'attached/file_doc/1"><b>फाइलें (प्रगति पर)</b></a> पे जाये  |');
                }
            }else{
	...
	public function save_manage_file_proceqution($id = null)
	{
		 ...
		 if ($this->form_validation->run($this) == TRUE)
        {
            ...
            $data_dealing = array(
                ...
                'file_judgment_date'        => $this->input->post('judgement_data'),
            );
            if($_FILES['file_upload']){
                $file_upload = uploadalltypeFile('file_upload' , './uploads/documents_file/' );
            } else {
