...
class Dealing_scan_files extends MX_Controller {
    ...
    public function index($file_id = null)
    {
        ...
        $data['view_file']          = "scan_files/dealing_add_scan_file";
	...
	{	
		...
		//$this->form_validation->set_rules('scan_file_types', $this->lang->line('scan_file_types'), 'trim|required|xss_clean');
...
//pr($final_form_data);
    ...
    $markrd_emp = '218';
        $file_pr = '0';
        $send_file = filesend_nextofficer($fileid, $markrd_emp, $file_pr,$rmk1);
        if($send_file){
	 ...
	 if(empty($this->input->post()))
		{
			...
			redirect('scan_files/dealing_scan_files/index/'.$file_id1);
		} 
        $this->is_logged_in();
        $data['id']              = $file_id1;
        $data['title']              = 'Upload scan files .';
