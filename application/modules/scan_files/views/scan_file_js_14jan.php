	...
	$("#error-meta_key").text();
	 ...
	 if( flag == 1){
	  ...
	  if( flag == 0){
	    ...
	    //alert("file_department_id"+file_department_id);
		var HTTP_PATH='<?php echo base_url(); ?>';
		$.ajax({
			type: "POST",
 ...
 function open_file(scan_id,file_path)
 {
  ...
  $(document).ready(function() {
        $("#btn-scan-file").click(function(){
			...
			var scan_opt ='';
			scan_opt = "<ul>";
			//console.log(scan_file_name.length);
    ...
    function  remove_scan_file(file_id){
		
		if (confirm("कृपया सुनिश्चित करें की आप इसे हटाना चाहते है| ")) {
				$( ".my_scan_file_"+file_id ).remove();
				var scan_files_id = $("#scan_files_id").val();
				//alert(scan_files_id.indexOf(file_id));
				if (scan_files_id.indexOf(file_id) >= 0){
					scan_files_id = scan_files_id.replace(file_id+',', "");
					//alert(scan_files_id);
...
function open_file_in_viewer( scan_id ,file_path){
	...
	// var content = "<object id='pdfObject' type='application/pdf' data='" + file_path + "' width='100%' height='100%' />";
	// $("#pdfDisplay").attr("title", file_path + " Preview");
	// $("#pdfDisplay").load(content);
	//$("#pdfDisplay").dialog({ width: 800, height: 400, resizable: true } });
	// return false;
