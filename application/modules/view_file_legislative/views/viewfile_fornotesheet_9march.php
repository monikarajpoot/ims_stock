...
if($files->multi_user_receiver_id !=null){
                                ...
                                <td width="100px" >
                                    ...
                                    if($senderemp[0]['role_id'] == '8' || $senderemp[0]['role_id'] == '37' || $senderemp[0]['role_id'] == '14'){
										$section_exp_emp = explode(',',getEmployeeSection());
										//pre($section_exp_emp);
										if(in_array(27,$section_exp_emp)){
											echo   $files->file_hardcopy_status == 'working' && (emp_session_id() == $files->file_received_emp_id) ? '<button onclick="open_model6('.$files->file_id.')" class="btn btn-sm btn-twitter btn-block rty6" value="'.$files->file_id.'" data-toggle="tooltip" data-original-title="Mark to Translator">अन्य सहायक को अंकित करें</button>' : false;
										}
                                        echo   $files->file_hardcopy_status == 'working' && (emp_session_id() == $files->file_received_emp_id || in_array(emp_session_id(),$multi_nm)) && $files->file_level_id != '31' ? '<button onclick="open_model2('.$files->file_id.')" value="'.$files->file_id.'"  class="btn btn-sm btn-block btn-twitter upperuser" data-toggle="tooltip" data-original-title="Send to officer">अधिकारी को भेजें</button> ' : false;
                                    ...
                                    echo   ($files->file_hardcopy_status == 'working' || $files->file_hardcopy_status == 'received' ) && (emp_session_id() == $files->file_received_emp_id || in_array(emp_session_id(),$multi_nm))  && $files->file_return == '1'  ? '<a onclick="return confirm_send()" href="'.base_url().'manage_file/dispatch_file_byso/'.$files->file_id.'" class="btn btn-block btn-sm btn-instagram rty1" value="'.$files->file_id.'" data-toggle="tooltip" data-original-title="Send to Dispatch">जावक शाखा में भेजें</a> ' : false;
