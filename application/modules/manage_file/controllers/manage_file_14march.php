   ...
   public function dispatch_for_close()
    {
        ...
        if($dispatchno){
			...
			$message = "Dear Sir/Mdm, File with UO No. ".$file_deatils[0]['file_uo_or_letter_no']." received from ".$file_deatils[0]['dept_name_en']." on ".get_date_formate($file_deatils[0]['file_uo_or_letter_date'],'d/m/Y')." is dispatched with Dispatch No ".$dispatched_no.".";
			//echo strlen($message);
			//pr($message);
        ...
        if ($file_id != null || emp_session_id() == $filedetails->file_sender_emp_id || emp_session_id() == $filedetails->file_received_emp_id)   {
			...
			if(emp_session_id() == $filedetails->file_sender_emp_id){
                ...
                {  
					...
					$rec_not = 'received';
					/*
					if($filedetails->final_draft_id != '' && ($filedetails->file_status == 'e' || $filedetails->file_status == 'e,p')) {
					$rec_not = 'not';
					}else{
					$rec_not = 'received';
					} */                   
            ...
            if($filedetails->file_from_section_id !='' && $filedetails->file_from_section_id !=0 && !in_array($filedetails->file_mark_section_id,$section_exp_1) && $empreciver[0]['role_id'] != '39' && !in_array($filedetails->file_from_section_id,$sender_section)) {
            ...
            if ($res1) {
                ...
                if(emp_session_id() == $filedetails->file_sender_emp_id && $filedetails->final_draft_id != '' && ($filedetails->file_status == 'e' || $filedetails->file_status == 'e,p')) {
                    $final_data = array(
                    ...
                    updateData(DRAFT, $final_data, array('draft_id' => $draft_data['draft_id']));
...
if($this->input->post('physical_file'))
	...
	public function ajax_file_reopen()
    {
        ...
        $filedetails =  getFileDetails($fileid);
        if ($fileid != null) {
            $receiver_id = empdetails(emp_session_id());
            $file_hardcopy_status = 'received';
            ...
            $file_data = array(
                'file_received_emp_id' => emp_session_id(),
                'file_sender_emp_id' => emp_session_id(),
