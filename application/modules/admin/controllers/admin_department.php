...
class Admin_department extends MX_Controller {
    ...
    public function addUpdatedepartment($deptid=NULL )
    {
		...
		//$this->form_validation->set_rules('dept_officer_nm', 'dept_officer_nm', 'required');
		//$this->form_validation->set_rules('dept_officer_number', 'dept_officer_number', 'numeric|min_length[10]|max_length[10]');
		//$this->form_validation->set_rules('dept_officer_email', 'dept_officer_email', 'valid_email');
        //$this->form_validation->set_rules('dept_name_en', 'dept_name_en', 'callback_alpha_dash_space');
        ...
        if ($this->form_validation->run($this) === TRUE) {
            ...
            if ($deptid) {
                ...
                $data = array(
				    ...
				    'dept_officer_nm' => trim($this->input->post('dept_officer_nm')),
					'dept_officer_number' => trim($this->input->post('dept_officer_number')),
					'dept_officer_email' => trim($this->input->post('dept_officer_email')),
					'department_default_no' => trim($this->input->post('dept_default_code')),  
                );
