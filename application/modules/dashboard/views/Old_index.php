...
<div class="box box-primary" id="divname1">
	<div class="box-header">
		<h3 class="box-title"><?php echo get_date_formate($todays); ?> को  जिन फाइलों पर आपने कार्य किया</h3>
	...
	<div class="box-body">
		<div class="row">
			<div class="col-md-12">	
				<form role="form" method="post" action="<?php echo base_url('dashboard')?>/common_dashboard/dashboard_report"  enctype="multipart/form-data">			
				<table class="table table-condensed text-center">
				<?php  $query_log = $CI->db->query("SELECT count(distinct(file_id)) as received FROM ft_file_logs where to_emp_id = '$userid' and date(`flog_created_date`) = '$today' ");
					$received = $query_log->row_array();
					$query_log = $CI->db->query("SELECT count(distinct(file_id)) as worked FROM ft_file_logs where from_emp_id = '$userid' and date(`flog_created_date`) = '$today' ");
					$worked = $query_log->row_array();
					?>
					<tr>
						<td><label>दिनांक से खोजे</label></td>
						<td><input type="text" class="form-control date1" name= "start_date" value="<?php echo get_date_formate($today,'d-m-Y'); ?>" placeholder="select date"/> <?php echo form_error('start_date') ; ?></td>
				...
				<div class="box box-primary">
					<div class="box-header">
						<h3 class="box-title">फाइल का पुनर्जीवन एवं एकीकरण</h3>
					</div>
					<div class="row">
							...
							<br/>
							<?php //pre($res);
							if(count($res)>0){  $hid2='display:block';}else{$hid2='display:none';} ?>
