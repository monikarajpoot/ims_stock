...
$contents .= ' <input type="text" class="date1" name="date6" placeholder="dd/mm/yyyy"  />';
} 
$contents .= ' के विरूद्ध  ';
if($is_genrate == true){
...
$contents .= ' '.$post_data['date7'];
	...
	$contents .= ' <input type="text" class="date1" name="date7" placeholder="dd/mm/yyyy" />';
}
$contents .= ' तक नियत ';
if($is_genrate == true){ 
...
if($is_genrate == true){
	$contents .= ' '.$post_data['writ_lists'].'</span>';
		...
		$contents .= ' <select name="writ_lists" >';
	foreach($writ_lists as $row){
		$contents .= '<option value="'.$row.'">'.$row.'</option>';
	}
	$contents .= '</select>';
