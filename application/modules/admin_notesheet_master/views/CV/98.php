...
$contents .= '<tr><td align="left"><table width="100%"><tr><td align="left">पृ0 क्रमांक 3(बी)/ ';
if($is_genrate == true){ 
    $contents .=  $post_data['number'];
} else {
    $contents .= '<input type="text" class="" name="number"  value=""/>';
}
$contents .= '/'.date("Y").'/'.$file_number.'/21-क(सि.)';
...
$contents .= '<tr><td align="left"><table><td vertical-align="top">विषय:-</td><td style="margin-left:3%"><span style="margin-left:%">';
if($is_genrate == true){ 
...
$contents .= ')</div></td></tr>';
$contents .= '<tr><td align="right" style="line-height:15px;"><div style="width:60%; text-align:center;">'.get_officer_dign($this->input->post('avar_secetroy')).'</div></td></tr>';
...
$contents .= '/'.date("Y").'/'.$file_number.'/21-क(सि.)</td><td align="right">भोपाल, दिनांक ';
if($is_genrate == true){ 
    $contents .=  get_date_formate($post_data['date4'],'d-m-Y');
} else {
    $contents .= '<input type="text" class="date1" name="date4" placeholder="dd-mm-yyyy" value="'.$today.'"/></div>';
