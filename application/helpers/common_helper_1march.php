...
function checkUserrole_by_id($id) {
    $CI = & get_instance();
...
function getemployeeRole($role_id = null) {
    $CI = & get_instance();
    if($role_id != null) {
...
function file_moniter_byuser($file_id = null , $file_moniter_byps = NULL, $emp_id = null) {
		if($emp_id == null){
...
function get_section_employee($section_id, $role_id = ''){
   ...
   function holidays_2015(){
	...
	function holidays_2016(){     
        $holidays = array(
           '2015-01-03', '2015-01-09',  '2015-01-10', '2015-01-16', '2015-01-17', '2015-01-24', '2015-01-26', '2015-01-31',
           /*'2015-02-01', '2015-02-03', '2015-02-08', '2015-02-14', '2015-02-15', '2015-02-17', '2015-02-21', '2015-02-22',
           '2015-03-01', '2015-03-06', '2015-03-01', '2015-03-08', '2015-03-14', '2015-03-15', '2015-03-21', '2015-03-22', '2015-03-28', '2015-03-29',
           '2015-04-02', '2015-04-03', '2015-04-05', '2015-04-11', '2015-04-12', '2015-04-14', '2015-04-18', '2015-04-19', '2015-04-21', '2015-04-26',
           '2015-05-03', '2015-05-04', '2015-05-09', '2015-05-10', '2015-05-16', '2015-05-17', '2015-05-24', '2015-05-31',
...
function total_work_by_officer_emp($task,$empid,$sectionid){
    $CI = & get_instance();
...
function count_total_ps_monitor_files(){
    $CI = & get_instance();
    $query1  = $CI->db->query("SELECT count(*) as totals FROM ft_files as files WHERE files.file_hardcopy_status!='close' and files.file_return!='2' and files.ps_moniter_date!='' and files.ps_moniter_date!='0000-00-00'");
    $recods= $query1->row_array();
...
function file_monitor_sms_sent($task,$empid,$fid,$filemove){
    ...
    $query0  = $CI->db->query("SELECT file_uo_or_letter_no,file_uo_or_letter_date FROM ft_files as files WHERE file_id = $fid and  files.file_hardcopy_status!='close' and files.file_return!='2' and files.ps_moniter_date!='' and files.ps_moniter_date!='0000-00-00'");
    $file_detail= $query0->row_array();
    echo $results_count= $query0->num_rows();
    
    $file_uo_no = $file_detail['file_uo_or_letter_no'].'/'.$file_detail['file_uo_or_letter_date'];
    ...
    $recods= $query1->row_array();
     ...
     $query6  = $CI->db->query("SELECT * FROM ft_file_monitor_sms where sms_emp_id=$empid and sms_file_id=$fid and sms_file_move=$filemove");
     $results6= $query6->num_rows();
    if($results6==0 && $results_count>0){
        $query4  = $CI->db->query("SELECT emp_full_name,emp_full_name_hi,emp_mobile_number FROM ft_employee where emp_status=1 and emp_is_retired =0 and emp_id=$empid");
        $emp_detail = $query4->row_array();
        $mobile_numbers= $emp_detail['emp_mobile_number'];
        //$mobile_numbers= '9893910866';
        $content = 'New file added in PS monitor. File UO no:'.$file_uo_no;
...
function check_file_exist_or_not(){
    if(isset($_GET['fmerge_type']) && $_GET['fmerge_type']!='' && $_GET['fnumber']!=''){
        ...
        if(isset($_GET['fmerge_type']) && $_GET['fmerge_type']=='fid'){
            ...
            $query1  = $CI->db->query("SELECT file_id FROM ft_files_section where section_id=1 and section_number=".$_GET['fnumber']);
            $recods= $query1->row_array();
            $results= $query1->num_rows();
            if(count($results)>0){
                $response= getFiledata($recods['file_id']);
...
function show_view_as_lvl(){
    ...
    }elseif($emp_role_lvl['emprole_level']==1 || $emp_role_lvl['emprole_level']==2 || $emp_role_lvl['emprole_level']==3 || $emp_role_lvl['emprole_level']==4 || $emp_role_lvl['emprole_level']==5){
...
function get_so_incharge_of_da($empid,$sectionid){
	$CI = & get_instance();    
    $query =   $CI->db->query("select emp_id_assign_by from ft_employee_permission_alloted where epa_module_name='files' and  epa_section_id=$sectionid and emp_id_assign_to = ".$empid);
    $emp_id = $query->row_array();
