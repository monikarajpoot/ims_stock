...
function empdetails($emp_id) {
    $CI = & get_instance();
    $CI->db->select('emp_id,role_id,emp_unique_id,emp_full_name,emp_full_name_hi,emp_email,emp_mobile_number,emp_section_id,emp_is_retired,emp_is_parmanent');
    $CI->db->where('emp_id', $emp_id);
    $CI->db->from(EMPLOYEES);
    $query = $CI->db->get();
    $data = $query->result_array();
...
function get_file_log_data($file_log_id = null){
	$CI = & get_instance();
...
function ps_monitor_marked_report($task = null , $today_condition = null , $empid = null){
    ...
    $today_c = '';
    $curr_emp = '';
    $userrole = checkUserrole_by_id($empid,true);
