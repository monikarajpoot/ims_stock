...
function verify_digital_sinature($log_id,$draft_content = null){
	...
	if($query->num_rows() > 0){
		...
		curl_close($ch);
		//pre($response);
		$show_data = '<b>'.$result['ds_emp_name'].'</b> <br/> '.get_datetime_formate($result['ds_create_datetime']);
		...
		if($response == '"success"'){
			...
			$return  = '<img src="'.base_url().'images/verify_sign.png" height="25px" width="30px" title="Signed by: '.$show_data_title.'"> <div style="font-size:12px; margin:0px;">Digitally Signed By <br/><span class="onlyprint" style="font-size:14px">'.$show_data.'</span></div>' ;
		} else { 
